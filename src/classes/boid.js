class Boid {
  ...
  getEdgeAvoidanceVector() {
    ...
    if (this.position[0] < offsetX) {
      ...
      edgeAvoidanceVector[0] = window.innerWidth - offsetX - this.position[0];
      this.avoidsEdge = true;
    }
    // Y component
    if (this.position[1] < offsetY) {
      ...
      edgeAvoidanceVector[1] = window.innerHeight - offsetY - this.position[1];
      this.avoidsEdge = true;
    }
  ...
  getAvoidVector(positions) {
    ...
    tooCloseBoids.forEach((position) => {
      diff = [position[0] - this.position[0], position[1] - this.position[1]];
